...
def Solver_TwoModesCoupledToMR_2(N,wa,wb,wr,kappa_a,kappa_b,gamma,n_th_r,E_a,E_b,galist,ohm_a_list):
    """
    This functions solves the steady-state for a system of two EM modes,
    A and B, coupled to single mechanical mode R, for different values of
    coupling strength g_a and driver detuning Delta_a.
    ...
    for k in range(len(galist)):
        ...
        for i in range(len(ohm_a_list)):
            ...
            #a_ss_2 = (Ea + (rho_p * a * Xr).tr() + (2 * ga**2 / wr) * (a * a.dag() * a * rho_ss).tr() + X) / ((ga**2 / wr) + 1j * (kappa_a/2) - 2 * chiAB * nb_ss - Delta_a)
            a_ss_2 = (E_a - ga * (rho_p * a * Xr).tr() - (2 * ga**2 / wr) * aada - X) / ((-ga**2 / wr) + 1j * (kappa_a/2) + 2 * chiAB * nb_ss - wa + Ohm_a)
